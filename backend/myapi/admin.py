...
class CustomUserAdmin(UserAdmin):
    model = CustomUser
    list_display = (
        'username', 'email', 'phone', 'area', 'role', 'approval_status', 'is_approved', 'is_staff', 'location'
    )
    list_editable = ('is_approved',)  # This makes is_approved editable in the list view!
    list_filter = ('role', 'is_approved', 'is_staff', 'is_superuser', 'is_active', 'location')
    actions = [approve_users]
    fieldsets = UserAdmin.fieldsets + (
        ('Additional Info', {'fields': ('phone', 'area', 'role', 'is_approved')}),
    )
...
class MemberApplicationAdmin(admin.ModelAdmin):
    list_display = (
        'name', 'email', 'phone', 'area', 'is_approved', 'approved_at'
    )
